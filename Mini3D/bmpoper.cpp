...
void doIt()
{
	...
	readBmp(readPath);
	//输出图像的信息
	std::cout << "width=" << bmpWidth << " height=" << bmpHeight << " biBitCount=" << biBitCount << std::endl;
	...
	//将图像左下角1/4部分置成黑色
	std::ofstream outfile("图像像素.txt", std::ios::in | std::ios::trunc);
